...
subroutine train_subroutine(infile)
      ...
      do itype = 1, ts%nTypes
         if (present(verbose)) then
            write(*,*) 'Saving the ', trim(ts%typeName(itype)), &
                 ' network to file : ', trim(inp%netFile(itype)) // trim(enumd)
         end if
         open(u_sav, file=trim(inp%netFile(itype))//trim(enumd), status='replace', &
...
end subroutine train_subroutine
  

end module aenet_train